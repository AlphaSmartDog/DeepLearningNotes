...
class Framework(object):
    def __init__(self, name, access, state_size, batch_size, action_size):
        self.Access = access
        self.AC = Agent(name, self.Access, state_size, batch_size, action_size)
        self.env = Account()
        self.name = name
    ...
    def run_episode(self, sess, t_max=8):
        ...
        while True:
            ...
            # adjust for smoother gradient
            buffer_reward.append(reward * 10)
            state = next_state

            if t - t_start == t_max or done:
                t_start = t
                next_state = np.transpose(next_state, [1, 0, 2])
    ...
    def get_bootstrap(self, sess, next_state, done):
        if done:
            ...
            gather = [next_state.shape[0]-1]
            terminal = self.AC.get_value(sess, next_state, gather)
        return terminal
